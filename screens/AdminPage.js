...
const OrderHistoryScreen = () => {
  ...
  const [currentStatus, setCurrentStatus] = useState(
    selectedOrder ? selectedOrder.상태 : "결제대기"
  );
            ...
            {ordersOnDate.map((order) => (
              <VStack space={3} key={order.id}>
                ...
                >
                  ...
                  <Text bold color={"green.700"} style={styles.textTypo4}>
                    자세히
                  </Text>
                </Pressable>
              </VStack>
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <VStack key={index} space={2}>
                ...
                {item.농도 && (
                  <Text style={styles.textTypo6}>농도: {item.농도}</Text>
                )}
                {item.베이스 && (
                  <Text style={styles.textTypo6}>베이스: {item.베이스}</Text>
                )}
                {item.용량 && (
                  <Text style={styles.textTypo6}>용량: {item.용량}</Text>
                )}
                {item.제형 && (
                  <Text style={styles.textTypo6}>제형: {item.제형}</Text>
                )}
                {item.케이스 && (
                  <Text style={styles.textTypo6}>케이스: {item.케이스}</Text>
                )}
                <Divider />
                <HStack
                  alignContent={"center"}
                ...
                >
                  ...
                  {selectedOrder && (
                    <Text color={"blue.500"} style={styles.textTypo4} bold>
                      {selectedOrder.보내는사람}
                    </Text>
                  )}
                </HStack>
                ...
                <VStack alignContent={"center"}>
                  ...
                  {selectedOrder && (
                    <Text color={"blue.500"} style={styles.textTypo4} bold>
                      {selectedOrder.도로명}, {selectedOrder.상세주소}{" "}
                    </Text>
                  )}
                </VStack>
                <HStack
